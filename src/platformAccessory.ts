...
export async function query(url:string, config:PlatformConfig, log:Logger, uid:string, onoff:string) {
  try {
    // 👇️ const response: Response
    const response = await fetch(url, {
      method: 'POST',
      ...
      body: JSON.stringify({'type':'light', 'uid' : uid, 'control':onoff}),
    });
...
export class SmartElifeAccessory {
  ...
  async getOn(): Promise<CharacteristicValue> {
    ...
    // eslint-disable-next-line max-len
    querystatus('https://node.apt.co.kr:7443/controls/device/status.ajax', this.platform.config, this.platform.log, this.accessory.context.device.uid)
      .then(result => {
